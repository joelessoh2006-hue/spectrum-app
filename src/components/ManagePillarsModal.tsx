          ...
          {editingCategory || isCreatingNew ? (
            <form onSubmit={handleSave} className="space-y-5">
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  {isCustomEmojiOrSymbol(formIconName) && (
                    <span className="text-[10px] font-mono px-2 py-0.5 rounded-full bg-[#6C5CE7]/15 text-[#6C5CE7] font-bold">
                      Émoji sélectionné : {getRawEmojiOrSymbol(formIconName)}
                    </span>
                  )}
                </div>

                {/* Saisie personnalisée d'émoji ou symbole libre */}
                <div className="mb-2.5 p-2.5 rounded-2xl bg-[var(--bg-surface-elevated)] border border-[var(--border-card)] space-y-2">
                  <div className="flex items-center gap-2">
                    <div className="w-8 h-8 rounded-xl bg-[var(--bg-surface)] border border-[var(--border-card)] flex items-center justify-center shrink-0 text-base shadow-xs">
                      {isCustomEmojiOrSymbol(formIconName) ? (
                  ...
                  {/* Suggestions d'émojis populaires */}
                  <div className="flex items-center gap-1.5 overflow-x-auto pb-1 pt-0.5 scrollbar-thin">
                    <span className="text-[10px] text-[var(--text-muted)] shrink-0 font-medium mr-1">
                      Populaires :
                    </span>
                    {POPULAR_PILLAR_EMOJIS.map((emoji) => {
                      ...
                      return (
                        <button
                          ...
                          className={`w-7 h-7 rounded-lg shrink-0 flex items-center justify-center text-sm transition-transform cursor-pointer ${
                            isSelected
                ...
                {/* Bibliothèque d'icônes vectorielles */}
